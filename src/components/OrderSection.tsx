...
const OrderSection = ({ isArabic }: OrderSectionProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // --- بداية الإصلاح المطلوب ---
      // التحقق من وجود الدالة قبل استدعائها
      if (typeof window.fbq === 'function') {
        window.fbq('track', 'Purchase', {value: 169.00, currency: 'AED'});
      }
      // --- نهاية الإصلاح المطلوب ---

      toast({
        title: "تم إرسال طلبك بنجاح! 🎉",
        description: "سيتم التواصل معك في أسرع وقت لتأكيد التفاصيل",
      });
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-start">
          ...
          <div className="space-y-8">
            <div className="bg-white rounded-3xl p-8 shadow-xl border-4 border-secondary">
              ...
              <div className="space-y-3 border-t pt-6">
                ...
                <div className="flex items-center gap-3">
                  <div className="bg-green-500 text-white rounded-full p-1"><Truck className="w-4 h-4" /></div>
                  <span>توصيل مجاني لكل الإمارات</span>
                </div>
                <div className="flex items-center gap-3">
                  <div className="bg-blue-500 text-white rounded-full p-1"><Shield className="w-4 h-4" /></div>
                  <span>ضمان سنتين شامل</span>
                </div>
               
                <div className="flex items-center gap-3">
                  <div className="bg-orange-500 text-white rounded-full p-1"><Clock className="w-4 h-4" /></div>
                  <span>توصيل سريع لكل الإمارات</span>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-green-50 border-2 border-green-200 rounded-2xl p-4 text-center">
                ...
                <div className="font-bold text-green-800">دفع آمن</div>
                <div className="text-sm text-green-600">عند الاستلام</div>
              </div>
              <div className="bg-blue-50 border-2 border-blue-200 rounded-2xl p-4 text-center">
                <Star className="w-8 h-8 text-blue-600 mx-auto mb-2" />
                <div className="font-bold text-blue-800">جودة مضمونة</div>
                <div className="text-sm text-blue-600">احكم بنفسك</div>
              </div>
            </div>
            ...
            <div className="bg-gray-50 rounded-2xl p-6">
              ...
              <div className="flex justify-center gap-8">
                <div className="text-center">
                  ...
                  <div className="text-xs text-muted-foreground">أوروبي</div>
                </div>
                <div className="text-center">
                ...
                <div className="text-center">
                  <div className="text-2xl font-bold text-green-600">UAE</div>
                  <div className="text-xs text-muted-foreground">محلي</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-3xl p-8 shadow-xl border border-border">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                <label className="block text-sm font-semibold text-foreground mb-2"><User className="w-4 h-4 inline ml-2" />الاسم الكامل *</label>
                <input type="text" name="name" value={formData.name} onChange={handleInputChange} className="w-full px-4 py-3 border-2 border-border rounded-xl focus:border-primary focus:outline-none transition-colors text-right" placeholder="أدخل اسمك الكامل" required />
              </div>
              <div>
                <label className="block text-sm font-semibold text-foreground mb-2"><Phone className="w-4 h-4 inline ml-2" />رقم الهاتف *</label>
                <input type="tel" name="phone" value={formData.phone} onChange={handleInputChange} className="w-full px-4 py-3 border-2 border-border rounded-xl focus:border-primary focus:outline-none transition-colors text-right" placeholder="05xxxxxxxx" required />
              </div>
              <div>
                <label className="block text-sm font-semibold text-foreground mb-2"><MapPin className="w-4 h-4 inline ml-2" />المدينة *</label>
                <select name="city" value={formData.city} onChange={handleInputChange} className="w-full px-4 py-3 border-2 border-border rounded-xl focus:border-primary focus:outline-none transition-colors text-right" required>
                  {cities.map(city => (<option key={city} value={city}>{city}</option>))}
                </select>
              </div>
              <div>
                <label className="block text-sm font-semibold text-foreground mb-2"><MapPin className="w-4 h-4 inline ml-2" />العنوان بالتفصيل *</label>
                <input type="text" name="address" value={formData.address} onChange={handleInputChange} className="w-full px-4 py-3 border-2 border-border rounded-xl focus:border-primary focus:outline-none transition-colors text-right" placeholder="المنطقة، اسم الشارع، رقم المبنى" required />
              </div>
              <button
                type="submit"
                disabled={isSubmitting}
                className="w-full bg-secondary-gradient text-white font-bold text-lg py-4 rounded-xl shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-300 flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed"
              >
                {isSubmitting ? (
                  <>
                  ...
                  <>
                    ...
                    <span>اطلب الآن - 169 درهم</span>
                  </>
                )}
              </button>
              <p className="text-center text-sm text-muted-foreground">
                🔒 بياناتك محمية 100% | دفع آمن عند الاستلام | لا توجد رسوم خفية
              </p>
            </form>
          </div>
        </div>
        ...
        <div className="mt-16 bg-gradient-to-r from-green-500 to-emerald-500 text-white rounded-3xl p-8 text-center">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            <div>
              ...
              <h4 className="font-bold mb-2">ضمان سنتان</h4>
              <p className="text-green-100">لا يشمل سوء الإستخدام</p>
            </div>
            <div>
              <Truck className="w-12 h-12 mx-auto mb-4" />
              <h4 className="font-bold mb-2">توصيل سريع ومجاني</h4>
              <p className="text-green-100">لكل الإمارات</p>
            </div>
            <div>
              <Star className="w-12 h-12 mx-auto mb-4" />
              <h4 className="font-bold mb-2">جودة مضمونة</h4>
              <p className="text-green-100">منتج أصلي بشهادات دولية</p>
            </div>
          </div>
        </div>
      </div>
